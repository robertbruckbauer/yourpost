 ...
 */
public class UprHandler implements ParserEventHandler
{
   /**
    * Constructor.
    */
   public UprHandler(ParserEventHandler handler)
   {
   ...
   private void processCommands()
   {
      ...
      while (true)
      {
         ...
         ParserEvent event = events.get(endIndex);
         switch (event.getType())
         {
            case GROUP_START_EVENT:
            {
   ...
   private final ParserEventHandler handler;
   private int groupCount = 1;
   private boolean complete;
   private final List<ParserEvent> events = new ArrayList<ParserEvent>();
}
